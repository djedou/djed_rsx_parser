...
use combine::combinator::{between, many, none_of, optional, parser, token, r#try, value, many1};
    ...
    I: Stream<Item = char>
{
    choice!(
        ...
        r#try(parser(rs_char).map(|v| RSXRawCodeFragment::Tokens(format!("'{}'", v.0)))),
        r#try(parser(rs_string).map(|v| RSXRawCodeFragment::Tokens(format!("\"{}\"", v.0)))),
